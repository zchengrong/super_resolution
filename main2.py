...
def process(model: ControlLDM, control_imgs: List[np.ndarray], steps: int, t_max: float, t_min: float, strength: float, color_fix_type: str, tiled: bool, tile_size: int, tile_stride: int) -> Tuple[List[np.ndarray]]:
    """
    ...
    if not tiled:
        samples = sampler.sample_ccsr(steps=steps, t_max=t_max, t_min=t_min, shape=shape, cond_img=control, positive_prompt="", negative_prompt="", x_T=x_T, cfg_scale=1.0, color_fix_type=color_fix_type)
    else:
        samples = sampler.sample_with_mixdiff_ccsr(tile_size=tile_size, tile_stride=tile_stride, steps=steps, t_max=t_max, t_min=t_min, shape=shape, cond_img=control, positive_prompt="", negative_prompt="", x_T=x_T, cfg_scale=1.0, color_fix_type=color_fix_type)
...
def main(image_url, sr_xn):
    ...
    x = np.array(x)
    preds = process(model=model, control_imgs=[x], steps=steps, t_max=t_max, t_min=t_min, strength=1, color_fix_type=color_fix_type, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
async def run(item: SRItem):
    sr_image_url_list = []
    image_obj = read_image(item.sr_input)
    lq = Image.open(image_obj).convert("RGB")
    if item.sr_xn == 2:
        original_width, original_height = lq.size
        new_width = original_width // 2
    ...
    x = np.array(x)
    preds = process(model, control_imgs=[x], steps=steps, t_max=t_max, t_min=t_min, strength=1, color_fix_type=color_fix_type, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
    sr_image_url = put_image(bucket=sr_bucket, file_name=generate_uuid(), obj=preds[0])
    print(f"sr runtime is {time.time() - start_time}")
    return {"sr_image_url": sr_image_url}


if __name__ == '__main__':
    # uvicorn.run(app, host="0.0.0.0", port=4562)
    image_url_list = [
        # "test/1709628055.1247382.png",
        "test/512_image/12.png",
        # "test/512_image/13.png",
        # "test/512_image/14.png",
        # "test/512_image/15.png",
        # "test/512_image/16.png",
        # "test/512_image/17.png",
        # "test/512_image/18.png",
        # "test/512_image/19.png",
        # "test/512_image/20.png",
    ]
    sr_xn = 2
    # with concurrent.futures.ThreadPoolExecutor() as executor:
    #     all_task = [executor.submit(main, i, sr_xn) for i in image_url_list]
    #     result = [i.result() for i in all_task]
    #     print(result)
    sr_result_url = main(image_url_list[0], sr_xn)
    print(sr_result_url)
