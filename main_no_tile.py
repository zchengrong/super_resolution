...
def main(image_url, sr_xn):
    ...
    # if x.size[0] > 2048 or x.size[1] > 2048:
    #     sr_image_url_list = []
    #     fragment_image = []
    #     width, height = x.size
    #     # 计算分割后的行数和列数
    #     rows = height // 2048 + (height % 2048 != 0)
    #     cols = width // 2048 + (width % 2048 != 0)
    #
    #     sub_images = []
    ...
    #             left = col * 2048
    #             upper = row * 2048
    #             right = min((col + 1) * 2048, width)
    #             lower = min((row + 1) * 2048, height)
    #             sub_image = x.crop((left, upper, right, lower))
    #             img_obj = np.array(sub_image)
    #             # TODO 这里可以做成多线程
    #             preds = process(model, [img_obj], steps=steps, t_max=t_max, t_min=t_min, strength=1,
    #                             color_fix_type=color_fix_type, tiled=tiled, tile_size=tile_size,
    #                             tile_stride=tile_stride)
    #
    #             sub_images.append(preds[0])
    #
    #     new_width = cols * 2048
    #     new_height = rows * 2048
    #     stitched_image_array = np.zeros((new_height, new_width, 3), dtype=np.uint8)
    #
    #     for i, sub_image_array in enumerate(sub_images):
    #         col_index = i % cols
    #         row_index = i // cols
    #         left = col_index * 2048
    #         upper = row_index * 2048
    #         stitched_image_array[upper:upper + 2048, left:left + 2048] = sub_image_array
    #
    #     url = put_image("test", generate_uuid(), stitched_image_array)
    #     return url
    # else:
    x = np.array(x)
    preds = process(model, [x], steps=steps, t_max=t_max, t_min=t_min, strength=1, color_fix_type=color_fix_type,
...
if __name__ == '__main__':
    ...
    # image_url = "test/1024_image/1709713346.806274.png"
