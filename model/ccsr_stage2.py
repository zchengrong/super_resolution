...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        gt, x, c = super().get_input(batch, self.first_stage_key, *args, **kwargs)
        ...
        return gt, x, dict(c_crossattn=[c], c_latent=[c_latent], lq=[lq], c_concat=[control])
    ...
    def log_images(self, batch, sample_steps=50):
        log = dict()
        gt, z, c = self.get_input(batch, self.first_stage_key)
        c_lq = c["lq"][0]
    ...
    def configure_optimizers(self):
        ...
        params_decode = list(self.first_stage_model.post_quant_conv.parameters()) + list(self.first_stage_model.decoder.parameters())
        # discriminator
        params_discriminator = list(self.decoder_loss.discriminator.parameters())
        if not self.sd_locked:
            params += list(self.model.diffusion_model.output_blocks.parameters())
            params += list(self.model.diffusion_model.out.parameters())
        opt = torch.optim.Adam(params_decode, lr=lr, betas=(0.5, 0.9))
        opt2 = torch.optim.Adam(params_discriminator, lr=lr, betas=(0.5, 0.9))
        return opt, opt2
